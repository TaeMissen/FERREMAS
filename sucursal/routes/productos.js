...
// Ruta para recibir productos de la casa matriz
...
// Ruta para enviar productos a la casa matriz
router.post('/enviar', async (req, res) => {
  ...
  try {
    ...
    if (producto && producto.cantidad >= cantidad) {
      // Actualizar la cantidad del producto en la sucursal
      ...
      // Enviar producto a la casa matriz
      const response = await axios.post('http://localhost:3000/productos/recibir', {
        id,
        cantidad,
      ...
      res.json({ message: 'Producto enviado con éxito a la casa matriz', data: response.data });
    } else {
